...
const
  INVALID_HANDLE_VALUE = TWMHandle(-1);
...
其它:调用失败，具体参数“错误码定义”。
==================================================================================*)
  WM_OpenDevice: function (nDevIndex: Integer; var DevHandle: TWMHandle): Integer; stdcall;
...
其它:调用失败，具体参数“错误码定义”。
==================================================================================*)
  WM_CloseDevice: function (DevHandle: TWMHandle): Integer; stdcall;
...
其它:调用失败，具体参数“错误码定义”。
==================================================================================*)
  WM_GetSerialNumber: function (DevHandle: TWMHandle; DeviceSN: PByte): Integer; stdcall;
...
其它:调用失败，具体参数“错误码定义”。
==================================================================================*)
  WM_GetImage: function (DevHandle: TWMHandle; nTimeOut: Integer;
...
var
  ...
  hLib: HModule=0;
const
  DLLMODULE0 = {$IFDEF MSWINDOWS}'WMRAPI.dll'{$ENDIF}{$IFDEF UNIX}'libwmrapi.so'{$ENDIF};
...
begin
  ...
  begin
    ...
    begin
      ...
      try
        ...
        hLib := LoadLibrary(PChar(sLibDLL));
      finally
  ...
  begin
    ...
    if iRefCount=0 then
    begin
      WM_Init          := GetProcAddress(hLib, 'WM_Init');
      WM_Free := GetProcAddress(hLib, 'WM_Free');
      WM_GetDeviceCount := GetProcAddress(hLib, 'WM_GetDeviceCount');
      WM_OpenDevice := GetProcAddress(hLib, 'WM_OpenDevice');
      WM_CloseDevice := GetProcAddress(hLib, 'WM_CloseDevice');
      WM_GetSerialNumber      := GetProcAddress(hLib, 'WM_GetSerialNumber');
      WM_GetImageInfo     := GetProcAddress(hLib, 'WM_GetImageInfo');
      WM_GetImage    := GetProcAddress(hLib, 'WM_GetImage');
      WM_Extract    := GetProcAddress(hLib, 'WM_Extract');
      {$IFDEF WEVI2}
      WM_GenTemplateWithImage := GetProcAddress(hLib, 'WM_GenTemplateWithImage');
      {$ELSE}
      WM_GenTemplate       := GetProcAddress(hLib, 'WM_GenTemplate');
      {$ENDIF}
      WM_Verify       := GetProcAddress(hLib, 'WM_Verify');
      WM_RawToBMP       := GetProcAddress(hLib, 'WM_RawToBMP');
